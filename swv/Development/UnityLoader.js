...
function DecompressAndLoadFile(url, onload, onprogress)
{
	...
	xhr.onerror = function() {
		...
		{
			alert ("It seems your browser does not support running this content from local drives. Please upload it to an http server, or try a different browser.");
...
function CompatibilityCheck()
{
    ...
    if (!1 && !hasWebGL)
    {
        alert("You need a browser which supports WebGL to run this content. Please try installing Firefox.");
        window.history.back();                
    }
    // Show warnings if needed.
    else if (mobile)
    {
        if (!confirm("Please note that this learning experience is not currently supported on mobiles. Press Ok if you wish to continue anyway."))
            window.history.back();        
    }
    else if (browser.indexOf("Firefox") == -1 && browser.indexOf("Chrome") == -1 && browser.indexOf("Safari") == -1)
    {
        if (!confirm("Please note that your browser is not currently supported for this learning content. Try installing Firefox or Chrome, or press Ok if you wish to continue anyway."))
            window.history.back();
    }
}
...
if (typeof window.onerror != 'function')
{
    window.onerror = function UnityErrorHandler(err, url, line)
    {
        ...
        {
            alert ("An exception has occured. Please notify technical support.");
            return;
        }
        if (err.indexOf("Cannot enlarge memory arrays") != -1)
        {
            alert ("The browser ran out of memory.");
            return;        
        }
        if (err.indexOf("Invalid array buffer length") != -1  || err.indexOf("Invalid typed array length") != -1 || err.indexOf("out of memory") != -1)
        {
            alert ("The browser could not allocate enough memory for the graphic content.");
            return;                
        }
        alert ("An error occured running the simulator. See your browser's JavaScript console for more info. The error was:\n"+err);
    }
}
...
Module.monitorRunDependencies = function(left) {
  ...
  Module.setStatus(left ? 'Decoding... (' + (this.totalDependencies-left) + '/' + this.totalDependencies + ')' : 'All transmissions received.');
};
Module.setStatus('Receiving transmissions... (0.0/1)');
...
(function() {
 var loadPackage = function(metadata) {
    ...
    var PACKAGE_NAME = 'swv.data';
    var REMOTE_PACKAGE_BASE = 'swv.data';
    ...
    var REMOTE_PACKAGE_NAME = typeof Module['locateFile'] === 'function' ?
      ...
      var REMOTE_PACKAGE_SIZE = 10685869;
      var PACKAGE_UUID = '33b2b03b-4153-47fb-8c30-3288502f3fb7';
...
Module['FS_createPath']('/Managed/mono', '2.0', true, true);
    ...
    DataRequest.prototype = {
      ...
      new DataRequest(0, 7014287, 0, 0).open('GET', '/data.unity3d');
    new DataRequest(7014287, 7016147, 0, 0).open('GET', '/methods_pointedto_by_uievents.xml');
    new DataRequest(7016147, 7025284, 0, 0).open('GET', '/preserved_derived_types.xml');
    new DataRequest(7025284, 9783272, 0, 0).open('GET', '/Il2CppData/Metadata/global-metadata.dat');
    new DataRequest(9783272, 10658244, 0, 0).open('GET', '/Resources/unity_default_resources');
    new DataRequest(10658244, 10685869, 0, 0).open('GET', '/Managed/mono/2.0/machine.config');
    ...
    function processPackageData(arrayBuffer) {
      ...
      DataRequest.prototype.byteArray = byteArray;
          ...
          Module['removeRunDependency']('datafile_swv.data');

    };
    Module['addRunDependency']('datafile_swv.data');
